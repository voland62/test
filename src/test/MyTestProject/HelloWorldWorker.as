package test.MyTestProject
{
	...
	public class HelloWorldWorker extends Sprite
	{
		...
		private function log(message : *, ...arr) : void
		{
			tf.appendText( String(message) + "\n");
			trace (message);
		}
		...
		public function HelloWorldWorker()
		{
			...
			log( "Worker.isSupported:" + Worker.isSupported ); 
		...
		{
			log("HelloWorldWorker.onMainToWorker(event)");
		...
		protected function onWorkerToMain(event : Event) : void
		{
			...
			log("[Worker] " + workerToMain.receive());
		}
	}
}